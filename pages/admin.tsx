...
const Admin = () => {
  ...
  const uploadPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success("Image succesfully uplaoded");
    } catch (error) {
      toast.error(`Upload failde ${error}`);
    }
    ...
    toast.promise(
      ...
      }),
      {
        ...
        error: `Upload failed 😥 Please try again ${error}`, // Include the error variable here
      }
    );
  };
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    const imageUrl = `http://${process.env.NEXT_PUBLIC_AWS_S3_BUCKET_NAME}.s3.amazonaws.com/${image[0]?.name}`;
